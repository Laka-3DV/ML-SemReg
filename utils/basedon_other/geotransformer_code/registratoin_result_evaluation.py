...
def compute_inlier_num_ratio(kpts_dst, kpts_src, transform, positive_radius=0.1, is_return_indices=False):
    r"""Computing the inlier ratio between a set of correspondences."""
    kpts_src = apply_transform(kpts_src, transform)
    residuals = np.sqrt(((kpts_dst - kpts_src) ** 2).sum(1))
    inlier_indice = residuals < positive_radius
    inlier_num = np.sum(inlier_indice) 
    inlier_ratio = inlier_num / len(inlier_indice)
    if is_return_indices:
        return inlier_num, inlier_ratio, inlier_indice
    return inlier_num, inlier_ratio 
